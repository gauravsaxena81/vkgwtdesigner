...
public class VkMenu extends MenuBar implements HasBlurHandlers{
	...
	public VkMenu(final Widget invokingWidget, IWidgetEngine widgetEngine) {
		...
		List<String> attributesList = VkDesignerUtil.getEngine().getAttributesList(invokingWidget, widgetEngine);
		...
		if(widgetsList != null && widgetsList.size() > 0)
		{
			...
			Command widgetClickedCommand = new Command() {
				...
				public void execute() {
					...
					if(widget != null)
						VkDesignerUtil.addWidget(widget, (Panel)invokingWidget, VkMenu.this.top, VkMenu.this.left);
					hideMenu();
				}
			};
			addSeparator();
			...
			public void execute() {
				...
				draggingWidget.addMouseMoveHandler(new MouseMoveHandler() {
					...
					public void onMouseMove(MouseMoveEvent event) {
						DOM.setStyleAttribute(draggingWidget.getElement(), "width", event.getClientX() 
								- VkDesignerUtil.getCumulativeLeft(invokingWidget.getElement()) + "px");
						DOM.setStyleAttribute(draggingWidget.getElement(), "height", event.getClientY() 
								- VkDesignerUtil.getCumulativeTop(invokingWidget.getElement()) + "px");
					}
				});
				draggingWidget.addMouseUpHandler(new MouseUpHandler() {
					...
					public void onMouseUp(MouseUpEvent event) {
						...
						DOM.setStyleAttribute(invokingWidget.getElement(), "width", draggingWidget.getOffsetWidth() + "px");
						DOM.setStyleAttribute(invokingWidget.getElement(), "height", draggingWidget.getOffsetHeight() + "px");
