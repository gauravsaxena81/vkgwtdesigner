		...
		if(!closeJs.isEmpty())
		{
			closeHandlerRegistration = super.addCloseHandler(new CloseHandler<TreeItem>() {
				...
				public void onClose(CloseEvent<TreeItem> event) {
					...
					setUpTreeEvent(eventproperties, treeItem);
					VkStateHelper.getInstance().getEventHelper().executeEvent(closeJs, eventproperties);
				}
			});
		}
	}
		...
		if(!openJs.isEmpty())
		{
			openHandlerRegistration = super.addOpenHandler(new OpenHandler<TreeItem>() {
				...
				public void onOpen(OpenEvent<TreeItem> event) {
					...
					TreeItem treeItem = event.getTarget();
					setUpTreeEvent(eventproperties, treeItem);
					VkStateHelper.getInstance().getEventHelper().executeEvent(openJs, eventproperties);
				}
			});
		}
	}
